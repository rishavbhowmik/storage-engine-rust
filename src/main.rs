...
fn main() {
    let (chan_sender, chan_reciver): IORequestChan = sync_channel(2);
    ...
    println!("Sleeping for 4s");
    std::thread::sleep(std::time::Duration::from_millis(4000));
    println!("Woke Up");
    loop {
        let request = chan_reciver.recv_timeout(std::time::Duration::from_millis(1000));
        match request {
            Ok(request) => {
                println!("{:?}", request);
            }
